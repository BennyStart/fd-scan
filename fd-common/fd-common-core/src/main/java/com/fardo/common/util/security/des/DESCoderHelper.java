...
public class DESCoderHelper {
	 ...
	 * @throws Exception
	 */
	private static byte[] decryptNet(byte[] data, byte[] key) throws Exception {
		Cipher cipher = Cipher.getInstance(CIPHER_ALGORITHM_CBC);
		DESKeySpec desKeySpec = new DESKeySpec(key);
		SecretKeyFactory keyFactory = SecretKeyFactory.getInstance(KEY_ALGORITHM);
	    SecretKey secretKey = keyFactory.generateSecret(desKeySpec);
	 ...
	 * @throws Exception
	 */
	public static String decryptNetString(String data, String key) throws Exception {
		keyLength(key);
	    return new String(decryptNet(Base64.decodeBase64(data),key.getBytes(CharEncoding.UTF_8)),CharEncoding.UTF_8);
	}
	 ...
	 */
	private static void keyLength(String key) throws Exception{
		keyLength(key.getBytes(StandardCharsets.UTF_8));
	}

	private static void keyLength(byte[] key) throws Exception{
		if(key.length != 8){
