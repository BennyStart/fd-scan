...
public class DictAspect {
    ...
    // 定义切点Pointcut
    @Pointcut("execution(public * com.fardo.modules..*.*Controller.*(..))")
...
//        log.debug("解析注入JSON数据  耗时"+(end-start)+"ms");
    ...
    private void parseDictText(Object result) {
        if (result instanceof ResultVo) {
            if (((ResultVo) result).getResults() instanceof IPage) {
                List<JSONObject> items = new ArrayList<>();
                for (Object record : ((IPage) ((ResultVo) result).getResults()).getRecords()) {
                    JSONObject item = getJsonObject(record);
                    items.add(item);
                }
                ((IPage) ((ResultVo) result).getResults()).setRecords(items);
            }else if(((ResultVo) result).getResults() instanceof List){
                //暂时不干啥
            }else if(((ResultVo) result).getResults() instanceof String){
                //暂时不干啥
            }else if(((ResultVo) result).getResults() instanceof Object){
                Object record = ((ResultVo) result).getResults();
                if(record != null) {
                    JSONObject item = getJsonObject(record);
                    ((ResultVo) result).setResults(item);
                }
            }
    ...
    private JSONObject getJsonObject(Object record) {
        ...
        //update-end--Author:scott  -- Date:20190603 ----for：解决继承实体字段无法翻译问题------
            if (field.getAnnotation(Dict.class) != null) {
                ...
                boolean conver = field.getAnnotation(Dict.class).conver();
