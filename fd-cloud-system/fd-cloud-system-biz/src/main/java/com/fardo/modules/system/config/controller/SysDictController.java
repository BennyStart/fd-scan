...
import com.fardo.common.util.SqlInjectionUtil;
...
import com.fardo.modules.system.config.model.SysDictTree;
...
import com.fardo.modules.system.config.vo.SysDictPage;
...
public class SysDictController {
	...
	public Result<IPage<SysDictEntity>> queryPageList(SysDictEntity sysDictEntity, @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
                                                      @RequestParam(name="pageSize", defaultValue="10") Integer pageSize, HttpServletRequest req) {
		Result<IPage<SysDictEntity>> result = new Result<IPage<SysDictEntity>>();
		QueryWrapper<SysDictEntity> queryWrapper = QueryGenerator.initQueryWrapper(sysDictEntity, req.getParameterMap());
		Page<SysDictEntity> page = new Page<SysDictEntity>(pageNo, pageSize);
		IPage<SysDictEntity> pageList = sysDictService.page(page, queryWrapper);
	...
	public Result<List<SysDictTree>> treeList(SysDictEntity sysDictEntity, @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
                                              @RequestParam(name="pageSize", defaultValue="10") Integer pageSize, HttpServletRequest req) {
		Result<List<SysDictTree>> result = new Result<>();
		LambdaQueryWrapper<SysDictEntity> query = new LambdaQueryWrapper<>();
		// 构造查询条件
		String dictName = sysDictEntity.getDictName();
		if(oConvertUtils.isNotEmpty(dictName)) {
			query.like(true, SysDictEntity::getDictName, dictName);
		}
		query.orderByDesc(true, SysDictEntity::getCreateTime);
		List<SysDictEntity> list = sysDictService.list(query);
	 ...
	 * @return
	 */
    @ApiOperation(value="根据字典编码获取字典数据", notes="根据字典编码获取字典数据")
	@RequestMapping(value = "/getDictItems/{dictCode}", method = RequestMethod.GET)
	public Result<List<DictModel>> getDictItems(@PathVariable String dictCode, HttpServletRequest request) {
	 ...
	 * @return
	 */
    @ApiOperation(value="字典编码&类型编码获取字典数据", notes="根据字典编码&类型编码获取字典数据")
	@RequestMapping(value = "/getDictText/{dictCode}/{key}", method = RequestMethod.GET)
	public String getDictText(@PathVariable("dictCode") String dictCode, @PathVariable("key") String key) {
		log.info(" dictCode : "+ dictCode);
		String text = null;
		try {
	...
	public Result<SysDictEntity> add(@RequestBody SysDictEntity sysDictEntity) {
		Result<SysDictEntity> result = new Result<SysDictEntity>();
		try {
			sysDictEntity.setCreateTime(DateUtils.getCurrentTime());
			sysDictEntity.setDelFlag(CommonConstant.DEL_FLAG_0);
	...
	public Result<SysDictEntity> edit(@RequestBody SysDictEntity sysDictEntity) {
		Result<SysDictEntity> result = new Result<SysDictEntity>();
		SysDictEntity sysdict = sysDictService.getById(sysDictEntity.getId());
		if(sysdict==null) {
			result.error500("未找到对应实体");
		}else {
			sysDictEntity.setUpdateTime(DateUtils.getCurrentTime());
			boolean ok = sysDictService.updateById(sysDictEntity);
			if(ok) {
				result.success("编辑成功!");
			}
		}
	 ...
	 */
	//@RequiresRoles({"admin"})
    @ApiOperation(value="数据字典删除", notes="数据字典删除")
	@RequestMapping(value = "/delete", method = RequestMethod.DELETE)
	@CacheEvict(value=CacheConstant.SYS_DICT_CACHE, allEntries=true)
	public Result<SysDictEntity> delete(@RequestParam(name="id",required=true) String id) {
		Result<SysDictEntity> result = new Result<SysDictEntity>();
	 ...
	 */
	//@RequiresRoles({"admin"})
    @ApiOperation(value="数据字典批量删除", notes="数据字典批量删除")
	@RequestMapping(value = "/deleteBatch", method = RequestMethod.DELETE)
	@CacheEvict(value= CacheConstant.SYS_DICT_CACHE, allEntries=true)
	public Result<SysDictEntity> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
		Result<SysDictEntity> result = new Result<SysDictEntity>();
	...
	public Result<?> refleshCache() {
		Result<?> result = new Result<SysDictEntity>();
		//清空字典缓存
	...
	public ModelAndView exportXls(SysDictEntity sysDictEntity, HttpServletRequest request) {
		// Step.1 组装查询条件
		QueryWrapper<SysDictEntity> queryWrapper = QueryGenerator.initQueryWrapper(sysDictEntity, request.getParameterMap());
		//Step.2 AutoPoi 导出Excel
		ModelAndView mv = new ModelAndView(new JeecgEntityExcelView());
		List<SysDictPage> pageList = new ArrayList<SysDictPage>();

		List<SysDictEntity> sysDictEntityList = sysDictService.list(queryWrapper);
		for (SysDictEntity dictMain : sysDictEntityList) {
			SysDictPage vo = new SysDictPage();
			BeanUtils.copyProperties(dictMain, vo);
			// 查询机票
			List<SysDictItemEntity> sysDictItemEntityList = sysDictItemService.selectItemsByMainId(dictMain.getId());
			vo.setSysDictItemEntityList(sysDictItemEntityList);
			pageList.add(vo);
		}
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				...
				for (int i=0;i< list.size();i++) {
					SysDictEntity po = new SysDictEntity();
					BeanUtils.copyProperties(list.get(i), po);
					po.setDelFlag(CommonConstant.DEL_FLAG_0);
					try {
						Integer integer = sysDictService.saveMain(po, list.get(i).getSysDictItemEntityList());
						if(integer>0){
							successLines++;
						}else{
							errorLines++;
	...
	public Result<List<SysDictEntity>> deleteList() {
		Result<List<SysDictEntity>> result = new Result<List<SysDictEntity>>();
		List<SysDictEntity> list = this.sysDictService.queryDeleteList();
		result.setSuccess(true);
	...
	@GetMapping(value = "/findKeyByInfo/{key}")
	public List<SysDictItemEntity> findKeyByInfo(@PathVariable("key") String key) {
		return sysDictItemService.findKeyByInfo(key);
	}
