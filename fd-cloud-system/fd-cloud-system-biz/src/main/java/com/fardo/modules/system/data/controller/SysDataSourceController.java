...
import com.fardo.common.aspect.annotation.AutoLog;
import com.fardo.common.system.base.controller.BaseController;
...
public class SysDataSourceController extends BaseController<SysDataSourceEntity, ISysDataSourceService> {
    ...
    public Result<?> queryPageList(
            SysDataSourceEntity sysDataSource,
            @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
            @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
            HttpServletRequest req
    ) {
        QueryWrapper<SysDataSourceEntity> queryWrapper = QueryGenerator.initQueryWrapper(sysDataSource, req.getParameterMap());
        Page<SysDataSourceEntity> page = new Page<>(pageNo, pageSize);
        IPage<SysDataSourceEntity> pageList = sysDataSourceService.page(page, queryWrapper);
        return Result.ok(pageList);
    }

    @GetMapping(value = "/options")
    public Result<?> queryOptions(SysDataSourceEntity sysDataSource, HttpServletRequest req) {
        QueryWrapper<SysDataSourceEntity> queryWrapper = QueryGenerator.initQueryWrapper(sysDataSource, req.getParameterMap());
        List<SysDataSourceEntity> pageList = sysDataSourceService.list(queryWrapper);
        JSONArray array = new JSONArray(pageList.size());
        for (SysDataSourceEntity item : pageList) {
    ...
    public Result<?> add(@RequestBody SysDataSourceEntity sysDataSource) {
    ...
    public Result<?> edit(@RequestBody SysDataSourceEntity sysDataSource) {
        SysDataSourceEntity d = sysDataSourceService.getById(sysDataSource.getId());
    ...
    public Result<?> deleteBatch(@RequestParam(name = "ids") String ids) {
        ...
        idList.forEach(item->{
            SysDataSourceEntity sysDataSource = sysDataSourceService.getById(item);
            DataSourceCachePool.removeCache(sysDataSource.getCode());
        });
    ...
    public ModelAndView exportXls(HttpServletRequest request, SysDataSourceEntity sysDataSource) {
        return super.exportXls(request, sysDataSource, SysDataSourceEntity.class, "多数据源管理");
    }
    ...
    public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
        return super.importExcel(request, response, SysDataSourceEntity.class);
    }
