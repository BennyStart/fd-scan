...
public class SysLogController {
	...
	public Result<IPage<SysLogEntity>> queryPageList(SysLogEntity syslog, @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
													 @RequestParam(name="pageSize", defaultValue="10") Integer pageSize, HttpServletRequest req) {
		Result<IPage<SysLogEntity>> result = new Result<IPage<SysLogEntity>>();
		QueryWrapper<SysLogEntity> queryWrapper = QueryGenerator.initQueryWrapper(syslog, req.getParameterMap());
		Page<SysLogEntity> page = new Page<SysLogEntity>(pageNo, pageSize);
		//日志关键词
	...
	public Result<SysLogEntity> delete(@RequestParam(name="id",required=true) String id) {
		Result<SysLogEntity> result = new Result<SysLogEntity>();
	...
	public Result<SysRoleEntity> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
	...
	public Result<?> add(@RequestBody JSONObject jsonObject) {
		...
		SysLogEntity log = JSON.parseObject(jsonObject.toJSONString(), SysLogEntity.class);
